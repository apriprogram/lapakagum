  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 sm:grid-cols-3">
      <div className="rounded-[16px] border border-slate-200/60 bg-white p-5">
        ...
        <p className="mt-2 text-2xl font-bold text-slate-900">{currency(sales)}</p>
      </div>
      <div className="rounded-[16px] border border-slate-200/60 bg-white p-5">
        <p className="text-[11px] font-medium uppercase tracking-wider text-slate-500">Total Pembelian</p>
        <p className="mt-2 text-2xl font-bold text-slate-900">{currency(purchase)}</p>
      </div>
      <div className="rounded-[16px] border border-slate-200/60 bg-white p-5">
        <p className="text-[11px] font-medium uppercase tracking-wider text-slate-500">Laba / Rugi</p>
        <p className={`mt-2 text-2xl font-bold ${sales - purchase >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{currency(sales - purchase)}</p>
      </div>
    </div>
    <div className="grid gap-6">
      <section className="surface overflow-hidden"><div className="flex flex-col gap-3 border-b border-slate-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-2"><BarChart3 className="size-4 text-emerald-600" /><div><h2 className="text-sm font-semibold text-blue-950">Grafik penjualan</h2><p className="mt-1 text-[11px] text-slate-400">Perbandingan pemasukan dan pengeluaran harian</p></div></div><div className="flex items-center gap-4"><div className="hidden sm:flex items-center gap-4 text-[10px] text-slate-500 mr-2"><span className="flex items-center gap-2"><i className="size-2.5 rounded-sm bg-emerald-500" /> Pemasukan</span><span className="flex items-center gap-2"><i className="size-2.5 rounded-sm bg-rose-500" /> Pengeluaran</span></div></div></div><div className="p-5 sm:p-6"><div className="grid grid-cols-[44px_1fr] gap-3"><div className="flex h-72 flex-col justify-between pb-8 text-right text-[10px] text-slate-400"><span>{currency(max)}</span><span>{currency(max*.75)}</span><span>{currency(max*.5)}</span><span>{currency(max*.25)}</span><span>Rp 0</span></div><div className="relative h-72"><div className="pointer-events-none absolute inset-x-0 top-0 flex h-[calc(100%-32px)] flex-col justify-between">{Array.from({length:5},(_,index)=><span key={index} className="block border-t border-slate-100" />)}</div><div className="absolute inset-0 flex items-end gap-1.5 overflow-x-auto pb-8 px-1 scroll-smooth">{days.map((item)=><div key={item.label} className="group flex h-full min-w-[32px] flex-1 flex-col items-center justify-end hover:bg-slate-50/50 rounded-t-lg transition-colors"><div className="flex w-full h-full items-end justify-center gap-[2px]"><div title={`Tanggal ${item.label}\nPemasukan: ${currency(item.value)}`} style={{height:`${Math.max(1,(item.value/max)*100)}%`}} className="relative flex w-full max-w-[12px] rounded-t-[3px] bg-emerald-500 transition-all duration-300 group-hover:bg-emerald-600"><span className="pointer-events-none absolute -top-8 left-1/2 -translate-x-1/2 z-10 hidden whitespace-nowrap rounded-md bg-slate-900 px-1.5 py-1 text-center text-[9px] font-medium text-white group-hover:block">{currency(item.value)}</span></div><div title={`Tanggal ${item.label}\nPengeluaran: ${currency(item.cost)}`} style={{height:`${Math.max(1,(item.cost/max)*100)}%`}} className="relative flex w-full max-w-[12px] rounded-t-[3px] bg-rose-500 transition-all duration-300 group-hover:bg-rose-600"><span className="pointer-events-none absolute -top-8 left-1/2 -translate-x-1/2 z-10 hidden whitespace-nowrap rounded-md bg-slate-900 px-1.5 py-1 text-center text-[9px] font-medium text-white group-hover:block">{currency(item.cost)}</span></div></div><span className="mt-3 text-[10px] text-slate-400 group-hover:text-slate-600 font-medium">{item.label}</span></div>)}</div></div></div></div></section>
    </div>
    <div className="grid gap-6">
      <section className="surface p-5 sm:p-6">
        ...
        <div className="mt-5 space-y-4">
          <div className="flex justify-between border-b border-blue-50 pb-4 text-sm">
            ...
            <span className="font-semibold text-blue-950">{currency(sales)}</span>
          </div>
          <div className="flex justify-between border-b border-blue-50 pb-4 text-sm">
            <span className="text-slate-500">Total Pembelian</span>
            <span className="font-semibold text-red-700">{currency(purchase)}</span>
          </div>
